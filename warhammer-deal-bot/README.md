...
The page also includes a searchable Warhammer 40,000 and Horus Heresy miniature
catalog generated by `scripts/refresh-warhammer-product-catalog.py`. The source
is WarHub's versioned public product catalog; links and images point back to
Warhammer. Characters, likely resin product codes, weapons, and upgrade sets are
hidden by default but can be shown explicitly. Warhammer's storefront uses AWS
human verification for automated page requests, so the importer does not bypass
it or claim unverified US prices. Missing MSRP values must be entered when a
product is added. The catalog refresh workflow runs each Monday and publishes
