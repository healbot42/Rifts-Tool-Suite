...
class SitemapProductPageAdapter(SourceAdapter):
    ...
    def search_many(self, products: list[Product]) -> dict[str, list[Listing]]:
        """Traverse the catalog once and fetch each candidate page once."""
        ...
        for url in self._load_catalog():
            ...
            matched = matching_products(slug, matchers)
            if matched:
        ...
        for url, matched_products in candidates.items():
            ...
            for product in matched_products:
                if listing := self.parse_page(content, url, product):
                    found[product.id].append(listing)
        return found
...
class ValhallaCatalogAdapter(SourceAdapter):
    ...
    def search_many(self, products: list[Product]) -> dict[str, list[Listing]]:
        """Issue each distinct query once and share its results with matching products."""
