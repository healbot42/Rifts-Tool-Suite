...
const catalogPath = new URL(
  '../src/data/tw-devices/tw-devices.json',
  import.meta.url,
)
...
function canonicalLabel(rawLabel) {
  ...
  if (/penalt|limitation|vulnerabil/i.test(label))
    return 'Penalties / Limitations'
  if (
    /requirements?|spells? (?:needed|required)|creation spells|spell chains?/i.test(
      label,
    )
  )
  ...
  if (
    /construction cost|cost to make|initial p\.?p\.?e|creation cost/i.test(
      label,
    )
  )
    return 'Construction Cost'
  if (/acti\s*vate|re\s*charge|reload|charge and re\s*charge/i.test(label))
    return 'Activation / Reload Cost'
  if (/market|price|^cost$/i.test(label)) return 'Price'
  if (
    /m\.?d\.?c|main body|armor rating|^a\.?r\.?|special protection/i.test(label)
  )
    return 'Durability / Protection'
...
function concise(value) {
  return normalizeDice(value)
    .replace(
      /\bThe (?:character|user|wielder|operator) (?:of the (?:device|weapon) )?can\b/gi,
      'Can',
    )
...
function concisePrice(value) {
  const boundary = value.search(
    /\s+(?:Note:|TW Firearms|Techno-Wizard Bionics|Techno-Wizard Vehicles|Flare:|Grenade:|\d+\.\s+[A-Z][A-Za-z -]{2,35}:)/i,
  )
...
function mergeStatistic(statistics, statistic) {
  ...
  if (!previous.value.includes(statistic.value))
    previous.value = `${previous.value}; ${statistic.value}`
  if (statistic.details && !previous.details?.includes(statistic.details)) {
    previous.details = previous.details
      ? `${previous.details} ${statistic.details}`
      : statistic.details
  }
}
...
function splitDamage(value) {
  const marker = value.search(
    /\s+(?:Those struck|Those hit|Against |Additionally|Targets? (?:can|must|are|suffer)|Victims? |A successful save|Whatever is hit|Special:|The person using)/i,
  )
  return marker > 0
    ? {
        label: 'Damage',
        value: value.slice(0, marker).trim(),
        details: value.slice(marker).trim(),
      }
    : { label: 'Damage', value }
}
...
function extractPrefixPowers(description, firstHeadingStart) {
  ...
  const marker = prefix.search(
    /\b(?:has|have|offers?|provides?|performs?) (?:two|three|four|five|six|several|the following) (?:functions?|powers?|modes?)\b|\b\d+\)\s+/i,
  )
  if (marker < 0) return null
  const powerText = prefix
    .slice(marker)
    .replace(/^.*?\b(?:functions?|powers?|modes?)\.?\s*/i, '')
    .trim()
  return powerText.length > 20
    ? { label: 'Powers / Effects', value: powerText }
    : null
}
...
function extractStatistics(description) {
  ...
  for (let index = 0; index < headings.length; index += 1) {
    ...
    if (heading.label === 'Damage') {
      ...
      if (nextMode > 0) {
        ...
        mergeStatistic(statistics, {
          label: 'Powers / Effects',
          value: value.slice(nextMode).trim(),
        })
      } else mergeStatistic(statistics, splitDamage(value))
    } else {
      if (heading.label === 'Construction Requirements') {
        const constructionPpe = value.match(
          /^(\d[\d,]*)\s+P\.?P\.?E\.?(?:\s+points?)?[,.;]\s*|(?:,?\s+and\s+)(\d[\d,]*)\s+P\.?P\.?E\.?\s+points?\.?$/i,
        )
        if (constructionPpe) {
          mergeStatistic(statistics, {
            label: 'Construction Cost',
            value: `${constructionPpe[1] ?? constructionPpe[2]} P.P.E.`,
          })
          value = value
            .replace(constructionPpe[0], constructionPpe.index === 0 ? '' : '.')
            .trim()
        }
      }
  ...
  const prefixPower = extractPrefixPowers(
    description,
    headings[0]?.start ?? description.length,
  )
  if (prefixPower) mergeStatistic(statistics, prefixPower)

  if (!statistics.some((stat) => stat.label === 'Activation / Reload Cost')) {
    const activation = description.match(
      /(?:costs?|requires?)\s+([^.!]{0,100}(?:P\.?P\.?E\.?|I\.?S\.?P\.?)[^.!]{0,100})/i,
    )
    if (activation)
      statistics.push({
        label: 'Activation / Reload Cost',
        value: `Activation: ${concise(activation[1])}`,
      })
  }
  if (!statistics.some((stat) => stat.label === 'Duration')) {
    const duration = description.match(
      /duration\s+(?:is|of|lasts?)\s+([^.!]{1,180})/i,
    )
    if (duration)
      statistics.push({ label: 'Duration', value: concise(duration[1]) })
  }

  return statistics.sort(
    (left, right) =>
      STATISTIC_ORDER.indexOf(left.label) -
      STATISTIC_ORDER.indexOf(right.label),
  )
}
...
const damageFallbacks = {
  'mega-blades-splugorth-tw-item':
    "Equal to the weapon's normal S.D.C. damage, converted to Mega-Damage.",
  'tw-arrows': 'Varies by the selected Goblin Bomb spell effect.',
  'naut-vii-tw-grenades':
    'Sonic Blast mode: 4D6 M.D. in a 20 foot (6.1 m) blast diameter; Black Water mode uses the spell effect.',
  'tw-vampire-water-field':
    '3D6 Hit Point damage each time a vampire enters the water field.',
  'tw-storm-flare':
    '4D6x10 Hit Point damage every half melee round (7.5 seconds) that a vampire remains exposed to the storm.',
  'water-dagger-tww-1050-by-jason-richards':
    '1 S.D.C./Hit Point to most targets; 2D6 Hit Points to vampires.',
}
const damageOverrides = {
  'tw-naut-yii-sonic-wand': {
    label: 'Damage',
    value: '2D6 M.D. per sonic blast or 6D6 M.D. as one combined blast.',
    details:
      'Stun mode: save vs magic (14+). Failure causes 3D6 S.D.C. (2D4 bypasses armor), -6 to strike/parry/dodge, half Speed and attacks for 1D4 melee rounds. Also inflicts 2D6 S.D.C. as a blunt weapon.',
  },
}
...
for (const device of catalog.devices) {
  ...
  const forcedDamage =
    damageOverrides[device.id] ??
    (damageFallbacks[device.id]
      ? { label: 'Damage', value: damageFallbacks[device.id] }
      : null)
  if (
    forcedDamage &&
    (damageOverrides[device.id] ||
    ...
    device.statistics = device.statistics.filter(
      (statistic) => statistic.label !== 'Damage',
    )
    device.statistics.unshift(forcedDamage)
  }
  if (
    weaponCategories.has(device.category) &&
    !device.statistics.some((statistic) => statistic.label === 'Damage') &&
    damageFallbacks[device.id]
  ) {
    device.statistics.unshift({
      label: 'Damage',
      value: damageFallbacks[device.id],
    })
  }
  if (powerOverrides[device.id]) {
    device.statistics = device.statistics.filter(
      (statistic) => statistic.label !== 'Powers / Effects',
    )
    device.statistics.push(powerOverrides[device.id])
    device.statistics.sort(
      (left, right) =>
        STATISTIC_ORDER.indexOf(left.label) -
        STATISTIC_ORDER.indexOf(right.label),
    )
...
console.log(
  `Extracted standardized statistics for ${catalog.devices.filter((device) => device.statistics.length).length} of ${catalog.devices.length} devices.`,
)
