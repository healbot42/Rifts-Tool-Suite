...
function safeText(value) {
  return String(value ?? '').replace(/[\u2018\u2019]/g, "'").replace(/[\u201C\u201D]/g, '"')
    ...
    .replace(/\u00D7/g, 'x').replace(/\u00A0/g, ' ').replace(/[^\x09\x0A\x0D\x20-\x7E\xA0-\xFF]/g, '?')
}
...
export function buildDevicePdf(report, JsPdf, descriptionStore) {
  ...
  const paragraph = (text, options = {}) => {
    ...
    while (offset < lines.length) {
      let availableLines = Math.floor((page.height - page.bottom - y) / lineHeight)
      if (availableLines < 1) { addPage(); availableLines = Math.floor((page.height - page.bottom - y) / lineHeight) }
  ...
  row('Device type', report.state.singleUse ? 'Single-use device' : 'Permanent device')
  row('Total construction P.P.E.', `${number(report.summary.ppeConstruction)} P.P.E.`)
  row('Total activation cost', report.summary.allFunctionsRequireLeyLine
  ...
  row('Cost before gems', `${number(report.summary.constructionCreditsBeforeGems)} credits`)
  row('Total construction cost (includes gems)', `${number(report.summary.constructionCredits)} credits`)
  row('Construction skill-roll modifier', `${report.summary.skillRollModifier >= 0 ? '+' : ''}${number(report.summary.skillRollModifier)}%`)

  heading('Calculation audit')
  row('Base function construction P.P.E.', number(report.calculations.basePpeConstruction))
  row('Stored P.P.E.', `${number(report.state.storagePpe)} at ${number(report.state.storagePercentPerPoint)}% each`)
  row('Storage construction modifier', `+${number(report.calculations.storageModifier)} P.P.E.`)
  const selectedModifiers = (report.state.constructionModifiers || []).map(selection => {
    const modifier = CONSTRUCTION_MODIFIER_BY_ID.get(selection.id)
    const quantity = modifier?.repeatable ? Math.max(1, Number(selection.quantity) || 1) : 1
    return modifier ? `${modifier.label}: -${number(modifier.percent * quantity)}%${quantity > 1 ? ` (${quantity} x ${modifier.percent}%)` : ''}` : null
  }).filter(Boolean)
  row('Construction skill penalties', selectedModifiers.length ? selectedModifiers.join('; ') : 'None selected')
  row('Book penalties subtotal', `-${number(report.calculations.selectedConstructionModifierPercent)}%`)
  const selectedBonuses = (report.state.constructionBonuses || []).map(selection => {
    const bonus = CONSTRUCTION_BONUS_BY_ID.get(selection.id)
    const quantity = bonus?.repeatable ? Math.max(1, Number(selection.quantity) || 1) : 1
    return bonus ? `${bonus.label}: +${number(bonus.percent * quantity)}%${quantity > 1 ? ` (${quantity} x ${bonus.percent}%)` : ''}` : null
  }).filter(Boolean)
  row('Construction skill bonuses', selectedBonuses.length ? selectedBonuses.join('; ') : 'None selected')
  ...
  report.state.chains.forEach((chain, chainIndex) => {
    ...
    heading(`${chainIndex + 1}. ${chain.name || `Function ${chainIndex + 1}`}`, 2)
    const spellPpe = chain.spells.reduce((sum, spell) => sum + (Number(spell.ppe) || 0), 0)
    row('Power mode', modeNames[chain.mode] || chain.mode); row('Primary gem size', `${number(chain.primaryGemCarats)} carats`)
    row('Selected spell P.P.E. total', `${number(spellPpe)} P.P.E.`); row('Construction P.P.E.', `${number(result.ppeConstruction)} P.P.E.`)
    row('Activation cost', chain.mode === 'ley-only' ? 'Ley line required to function' : `${number(result.activation)} P.P.E.`)
    row('Gem cost for function', `${number(result.gemCost)} credits${report.state.singleUse ? ' (25% single-use rule applied)' : ''}`)
    chain.spells.forEach((spell, spellIndex) => {
      const catalog = spellByName.get(String(spell.name).toLowerCase()); const gem = RIFTS_GEMS[catalog?.gemId]
      const carats = spellIndex === 0 ? Number(chain.primaryGemCarats) || 0 : 1; const mode = selectedMode(spell, catalog)
      const modeDetail = mode ? `; mode: ${mode.label}${mode.inputLabel ? `; ${mode.inputLabel}: ${spell.ppeModeInput ?? mode.inputDefault}` : ''}${mode.note ? `; ${mode.note}` : ''}` : ''
      paragraph(`${spellIndex === 0 ? 'Primary' : 'Secondary'} - ${spell.name || 'Unnamed spell'}: ${number(spell.ppe)} P.P.E.${spell.ppeText ? ` (book: ${spell.ppeText})` : ''}${modeDetail}`, { bold: true, indent: 10, after: 3 })
      paragraph(gem ? `Gem: ${gem.name}; ${number(carats)} carat${carats === 1 ? '' : 's'} at ${number(gem.pricePerCarat)} credits per carat = ${number(carats * gem.pricePerCarat)} credits.` : 'Gem: no catalog match is available for this spell.', { indent: 10, size: 8.5, after: 5 })
    })
  })

  heading('Gem requirements')
  report.state.chains.forEach((chain, chainIndex) => chain.spells.forEach((spell, spellIndex) => {
    const catalog = spellByName.get(String(spell.name).toLowerCase()); const gem = RIFTS_GEMS[catalog?.gemId]
    const carats = spellIndex === 0 ? Number(chain.primaryGemCarats) || 0 : 1
    if (gem) paragraph(`${gem.name} - ${spellIndex === 0 ? 'primary focus gem' : 'secondary spell gem'} for ${spell.name} in ${chain.name || `Function ${chainIndex + 1}`}. ${number(carats)} carat${carats === 1 ? '' : 's'} required at ${number(gem.pricePerCarat)} credits per carat; raw value ${number(carats * gem.pricePerCarat)} credits.`, { after: 5 })
  }))
  if (report.state.singleUse) note('For a single-use device, the construction calculation charges 25% of the raw required gem value.')
  heading('Device notes'); paragraph(report.state.notes || 'No device notes were entered.')

  const descriptionByName = new Map(Object.values(descriptionStore?.spells || {}).map(entry => [String(entry.name).toLowerCase(), entry]))
  const usedNames = [...new Set(report.state.chains.flatMap(chain => chain.spells.map(spell => spell.name)).filter(Boolean))]
  heading('Used spell descriptions')
  usedNames.forEach(name => {
    const catalog = spellByName.get(String(name).toLowerCase()); const entry = descriptionByName.get(String(name).toLowerCase())
    heading(`${name}${catalog?.level ? ` (Level ${catalog.level})` : ''}`, 2)
    if (entry?.source) {
      const pages = Array.isArray(entry.source.pages) ? [...new Set(entry.source.pages)].join('-') : entry.source.pages
      note(`Source: ${entry.source.book || 'Rifts Book of Magic'}${pages ? `, page ${pages}` : ''}.`)
    }
    paragraph(entry?.description || 'A full description is not available in the local spell-description store.')
  })
