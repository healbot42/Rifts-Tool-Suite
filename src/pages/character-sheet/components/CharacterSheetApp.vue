...
<template>
  <div class="character-sheet">
    ...
    <section v-if="editTab==='class'&&activeOcc" class="panel class-panel"><div class="class-heading"><div><p class="eyebrow">Selected class</p><h2 v-tooltip="activeOcc.description" tabindex="0">{{ activeOcc.name }}</h2><p>{{ activeOcc.description }}</p></div><div class="skill-counters"><output v-tooltip="'O.C.C. Related Skills remaining at the current level. New selections begin at first-level proficiency.'" tabindex="0"><span>O.C.C. Related</span><strong>{{ relatedRemaining }} available / {{ relatedTotal }} total</strong></output><output v-tooltip="'Secondary Skills remaining at the current level. These receive no O.C.C. bonus, but may receive an I.Q. bonus.'" tabindex="0"><span>Secondary</span><strong>{{ secondaryRemaining }} available / {{ secondaryTotal }} total</strong></output></div></div>
    ...
    <section v-if="editTab==='skills'" class="panel skills-panel"><div class="skills-heading"><div><h2>Skills</h2><p>Categories start collapsed. Hover over or focus a skill name for details. A repeated skill is one shared record: editing it in any category updates every occurrence.</p></div><div v-if="activeOcc" class="skill-counters"><output v-tooltip="'O.C.C. Related Skills remaining / total at this level.'" tabindex="0"><span>O.C.C. Related</span><strong>{{ relatedRemaining }} / {{ relatedTotal }}</strong></output><output v-tooltip="'Secondary Skills remaining / total at this level.'" tabindex="0"><span>Secondary</span><strong>{{ secondaryRemaining }} / {{ secondaryTotal }}</strong></output></div></div>
      ...
      <details v-for="([category,ids]) in skillCategories" :key="category"><summary><span>{{ category }}</span><small>{{ ids.filter(id=>skillRecord(id).selected).length }} selected / {{ ids.length }}</small></summary>
