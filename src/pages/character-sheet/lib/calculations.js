...
export function attributeBonus(attribute, score) {
  ...
  if (attribute === 'ppInitiative' && value > 30)
    return Math.min(6, Math.floor((value - 28) / 3))
...
export function weightLimits(ps, strengthType = 'normal') {
  ...
  if (strengthType === 'robot')
    multiplier = value >= 40 ? 100 : value >= 17 ? 25 : 10
  if (strengthType === 'supernatural') multiplier = value >= 18 ? 50 : 20
  let carry = value * multiplier
  if (strengthType === 'normal' && value > 30)
    carry *= 1 + Math.floor((value - 30) / 5) * 0.3
...
export function hitPoints(
  ...
  return (
    Math.max(0, Number(pe) || 0) +
    Math.max(0, Number(firstLevelRoll) || 0) +
    (levelsAfterFirst ? Math.max(0, Number(laterLevelRolls) || 0) : 0) +
    (Number(bonuses) || 0)
  )
}
...
export function skillTotal(skill, record, iqBonus = 0) {
  ...
  const currentLevel = Math.max(
    learnedLevel,
    Number(record.currentLevel) || learnedLevel,
  )
