...
export function calculateModePpe(mode, inputValue) {
  const input = Math.max(
    mode.inputMin ?? 0,
    Number(inputValue ?? mode.inputDefault) || 0,
  )
  if (mode.calculation === 'add')
    return mode.ppe + input * (mode.multiplier ?? 1)
  if (mode.calculation === 'max')
    return Math.max(mode.ppe, input * (mode.multiplier ?? 1))
  if (mode.calculation === 'perUnit')
    return Math.ceil(input / mode.unit) * mode.ppePerUnit
  if (mode.calculation === 'perUnitAdd')
    return mode.ppe + Math.ceil(input / mode.unit) * mode.ppePerUnit
  return mode.ppe
}

export function calculateRequiredGemCost(
  chain,
  gemForSpell,
  primaryGemCarats = chain.primaryGemCarats,
) {
...
export function calculateChainBasePpe(
  state,
  chain,
  primaryGemCarats = chain.primaryGemCarats,
) {
  const spellPpe = chain.spells.reduce(
    (sum, spell) => sum + clampNumber(spell.ppe),
    0,
  )
...
export function calculateNetSkillRollModifier(penalties, bonuses, custom = 0) {
  return (
    (Number(bonuses) || 0) - (Number(penalties) || 0) + (Number(custom) || 0)
  )
}
...
export function calculateSelectionPercent(selections, catalogById) {
  return (selections || []).reduce((total, selection) => {
    ...
    const quantity = entry.repeatable
...
export function calculateConstructionCredits(
  constructionPpe,
  deviceLevel,
  formCost,
  gemCost,
  ...
  const beforeGems =
    clampNumber(constructionPpe) * 10 * clampNumber(deviceLevel, 1) +
    clampNumber(formCost)
  return { beforeGems, total: beforeGems + clampNumber(gemCost) }
}

export function calculateConstructionHours(
  state,
  constructionPpe,
  timeMultiplier = 1,
) {
  ...
  if (state.existingTechnology) {
    ...
    hours =
      (clampNumber(constructionPpe) / 10) * clampNumber(state.deviceLevel, 1)
  }
...
export function buildFinalSummary(values) {
  return {
    ...
    constructionCreditsBeforeGems: roundUp(
      values.constructionCreditsBeforeGems,
    ),
