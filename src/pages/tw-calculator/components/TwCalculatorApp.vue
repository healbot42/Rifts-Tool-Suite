...
function timeAdjustmentValue(adjustment, multiplier) {
  return adjustment ? `${adjustment.label} (x${credits(multiplier)})` : 'None'
}

function verticalAxisTransform(x, chartState, rotation) {
  const y = chartState.margin.top + chartState.plotHeight / 2
...
const spellByName = new Map(
  searchableSpells.map(({ spell, searchName }) => [searchName, spell]),
)
...
function modeCostLabel(mode) {
  if (mode.calculation === 'perUnit')
    return `${mode.ppePerUnit} per ${mode.unit}`
...
const savedTimeAdjustment = [
  ...(state.constructionModifiers || []).map((selection) =>
    CONSTRUCTION_MODIFIER_BY_ID.get(selection.id),
  ),
  ...(state.constructionBonuses || []).map((selection) =>
    CONSTRUCTION_BONUS_BY_ID.get(selection.id),
  ),
...
const basePpeConstruction = computed(() =>
  chainResults.value.reduce((sum, row) => sum + row.ppeConstruction, 0),
)
const storageModifier = computed(
  () =>
    (basePpeConstruction.value *
      clampNumber(state.storagePpe) *
      clampNumber(state.storagePercentPerPoint)) /
    100,
)
const selectedConstructionModifierPercent = computed(() =>
  calculateSelectionPercent(
    state.constructionModifiers,
    CONSTRUCTION_MODIFIER_BY_ID,
  ),
)
const selectedConstructionBonusPercent = computed(() =>
  calculateSelectionPercent(
    state.constructionBonuses,
    CONSTRUCTION_BONUS_BY_ID,
  ),
)
const totalSkillRollModifier = computed(() =>
  calculateNetSkillRollModifier(
...
const modifiedPpeConstruction = computed(
  () => basePpeConstruction.value + storageModifier.value,
)
const gemTotal = computed(() =>
  chainResults.value.reduce((sum, row) => sum + row.gemCost, 0),
)
const constructionCreditTotals = computed(() =>
  calculateConstructionCredits(
    modifiedPpeConstruction.value,
    state.deviceLevel,
    state.formCost,
    gemTotal.value,
  ),
)
const constructionCreditsBeforeGems = computed(
  () => constructionCreditTotals.value.beforeGems,
)
...
const chart = computed(() => {
  ...
  const metricsAt = (carat) => {
    const projectedChains = state.chains.map((chain, index) => {
      const ppeConstruction = chainBasePpe(
        chain,
        index === 0 ? carat : chain.primaryGemCarats,
      )
      const activation = calculateChainActivation(state, chain, ppeConstruction)
      return { ppeConstruction, activation }
    })
    const basePpe = projectedChains.reduce(
      (sum, row) => sum + row.ppeConstruction,
      0,
    )
    const storage =
      (basePpe *
    ...
    const projectedGemTotal =
      state.chains.reduce(
        (sum, chain, index) =>
          sum +
          requiredGemCost(chain, index === 0 ? carat : chain.primaryGemCarats),
        0,
      ) * (state.singleUse ? 0.25 : 1)
    return {
      price: Math.ceil(
        modifiedPpe * 10 * clampNumber(state.deviceLevel, 1) +
          projectedGemTotal +
          clampNumber(state.formCost),
      ),
      activation: Math.ceil(
        projectedChains.reduce((sum, row) => sum + row.activation, 0),
      ),
    }
  }
  ...
  const points = carats
    .filter((carat) => carat > 0)
    .map((carat) => ({ carat, ...metricsAt(carat) }))
  ...
  const scaleFor = (values) => {
    ...
    const max = Math.max(
      min + step,
      Math.ceil((rawMax + spread * 0.1) / step) * step,
    )
  ...
  return {
    ...
    yTicks: priceScale.ticks.map((value) => ({
      value,
      y: priceScale.position(value),
    })),
    activationTicks: activationScale.ticks.map((value) => ({
      value,
      y: activationScale.position(value),
    })),
...
function showChartTooltip(event, series) {
  ...
  const pointerX =
    ((event.clientX - bounds.left) * chart.value.width) / bounds.width
  ...
  const x =
    point.x + tooltipWidth + 18 > chart.value.width
      ? point.x - tooltipWidth - 12
      : point.x + 12
  const y =
    pointY - tooltipHeight - 12 < 0 ? pointY + 12 : pointY - tooltipHeight - 12
  chartTooltip.value = {
    series,
    point,
    x,
...
const selectedConstructionModifierMap = computed(
  () =>
    new Map(
      (state.constructionModifiers || []).map((selection) => [
        selection.id,
...
const selectedConstructionBonusMap = computed(
  () =>
    new Map(
      (state.constructionBonuses || []).map((selection) => [
        selection.id,
...
function removeOtherTimeAdjustments(selectedId) {
  state.constructionModifiers = (state.constructionModifiers || []).filter(
    (selection) => {
      const entry = CONSTRUCTION_MODIFIER_BY_ID.get(selection.id)
      return selection.id === selectedId || !entry?.timeExclusive
    },
  )
  state.constructionBonuses = (state.constructionBonuses || []).filter(
    (selection) => {
...
function toggleConstructionModifier(modifier, checked) {
  ...
  if (checked) {
    ...
    if (!selectedConstructionModifier(modifier.id))
      state.constructionModifiers.push({ id: modifier.id, quantity: 1 })
  } else {
    state.constructionModifiers = state.constructionModifiers.filter(
      (selection) => selection.id !== modifier.id,
    )
  }
}
function setConstructionModifierQuantity(id, value) {
  ...
  if (selection)
    selection.quantity = Math.max(1, Math.floor(Number(value) || 1))
}
...
function toggleConstructionBonus(bonus, checked) {
  ...
  if (checked) {
    ...
    if (!selectedConstructionBonus(bonus.id))
      state.constructionBonuses.push({ id: bonus.id, quantity: 1 })
  } else {
    state.constructionBonuses = state.constructionBonuses.filter(
      (selection) => selection.id !== bonus.id,
    )
  }
}
...
const constructionTimeMultiplier = computed(
  () => activeTimeAdjustment.value?.timeMultiplier ?? 1,
)
const constructionHours = computed(() =>
  calculateConstructionHours(
    state,
    modifiedPpeConstruction.value,
    constructionTimeMultiplier.value,
  ),
)
const totalActivation = computed(() =>
  chainResults.value.reduce((sum, row) => sum + row.activation, 0),
)
const hasLeyLineOnlyFunctions = computed(() =>
  state.chains.some((chain) => chain.mode === 'ley-only'),
)
const allFunctionsRequireLeyLine = computed(
  () =>
    !state.singleUse &&
    state.chains.length > 0 &&
    state.chains.every((chain) => chain.mode === 'ley-only'),
)
...
function addChain() {
  state.chains.push(makeChain(state.chains.length + 1))
}
function removeChain(id) {
  if (state.chains.length > 1)
    state.chains = state.chains.filter((chain) => chain.id !== id)
}
function addSpell(chain) {
  chain.spells.push(makeSpell(false))
}
function removeSpell(chain, id) {
...
function exportJson() {
  const blob = new Blob(
    [JSON.stringify({ ...state, calculated: summary.value }, null, 2)],
    {
    ...
    await downloadDevicePdf(
      {
        ...
        calculations: {
          ...
          selectedConstructionModifierPercent:
            selectedConstructionModifierPercent.value,
          selectedConstructionBonusPercent:
            selectedConstructionBonusPercent.value,
          totalSkillRollModifier: totalSkillRollModifier.value,
          constructionTimeAdjustment:
            activeTimeAdjustment.value?.label || 'None',
          constructionTimeMultiplier: constructionTimeMultiplier.value,
        },
        chains: chainResults.value.map(
          ({ ppeConstruction, activation, gemCost }) => ({
            ppeConstruction,
            activation,
            gemCost,
          }),
  ...
  <main class="tw-calculator-page">
    <header class="hero">
      <div>
        ...
        <p>
          Build spell chains and estimate P.P.E., activation, construction time,
          and credit cost.
        </p>
      </div>
      <div class="actions">
        ...
        >
          {{ pdfExporting ? 'Creating PDF...' : 'Export PDF' }}
        </button>
        <button
          class="secondary"
          :disabled="pdfExporting"
    ...
    <div class="app-layout">
      <div class="workspace">
        <section class="panel grid settings">
          ...
          <label
            >Form<input
              v-model="state.form"
              placeholder="Pistol, armor, vehicle…"
          /></label>
          <label
            >Device level<input
              v-model.number="state.deviceLevel"
              type="number"
              min="1"
          /></label>
          <label
            >Basic form cost (credits)<input
              v-model.number="state.formCost"
              type="number"
              min="0"
          /></label>
          <label class="check"
            ><input
              v-model="state.singleUse"
              type="checkbox"
            />
            Single-use device</label
          >
          <label class="check"
            ><input
              v-model="state.existingTechnology"
              type="checkbox"
            />
            Added to existing technology</label
          >
          <label
            v-if="state.existingTechnology"
            class="check"
            ><input
              v-model="state.creatorHasMechanicalSkill"
              type="checkbox"
            />
            Creator has appropriate Mechanical skill</label
          >
        </section>

        <section class="chains">
          <article
            v-for="(chain, chainIndex) in state.chains"
            :key="chain.id"
            class="panel chain"
          >
            <div class="chain-head">
              <input
                v-model="chain.name"
                class="title-input"
                :aria-label="`Function ${chainIndex + 1} name`"
              />
              <button
                class="danger"
                :disabled="state.chains.length === 1"
            ...
            <div class="grid chain-settings">
              <label
                >Primary gem carats<input
                  v-model.number="chain.primaryGemCarats"
                  type="number"
                  min="0.5"
                  step="0.5"
              /></label>
              <label
                >Required gems cost (credits)<input
                  :value="credits(requiredGemCost(chain))"
                  readonly
              /></label>
              <label
                >Power mode
            ...
            <div class="spell-list">
              <div
                v-for="(spell, spellIndex) in chain.spells"
                :key="spell.id"
                class="spell-row"
              >
                <span class="badge">{{
                  spellIndex === 0 ? 'Primary' : 'Secondary'
                }}</span>
                <div class="spell-picker">
                  ...
                  >
                    ...
                    >
                      ...
                      <small
                        >Level {{ option.level }} ·
                        {{ option.ppeText || option.ppe }} P.P.E.</small
                      >
                    </button>
                    <p
                      v-if="matchingSpells(spell).length === 0"
                      class="no-spells"
                    >
                      No matching spells.
                    </p>
                  ...
                  <p
                    v-if="gemForSpell(spell)"
                    class="gem-note"
                  >
                    Required gem: <strong>{{ gemForSpell(spell).name }}</strong>
                    <span
                      >{{ credits(gemForSpell(spell).pricePerCarat) }} credits
                      per carat</span
                    >
                  </p>
                  <div
                    v-if="selectedSpell(spell)?.ppeModes?.length"
                    class="spell-mode"
                  >
                    <label
                      >Cost mode
                      <select
                        :value="activePpeMode(spell)?.id"
                        @change="setSpellMode(spell, $event.target.value)"
                      >
                        <option
                          v-for="mode in selectedSpell(spell).ppeModes"
                          :key="mode.id"
                          :value="mode.id"
                        >
                    ...
                    <label v-if="activePpeMode(spell)?.inputLabel">
                      ...
                      <input
                        ...
                        :value="
                          spell.ppeModeInput ??
                          activePpeMode(spell).inputDefault
                        "
                        @input="setSpellModeInput(spell, $event.target.value)"
                      />
                    </label>
                    <p
                      v-if="activePpeMode(spell)?.note"
                      class="mode-note"
                    >
                      {{ activePpeMode(spell).note }}
                    </p>
                  </div>
                </div>
                <div class="ppe-field">
                  <label class="inline"
                    >P.P.E.<input
                      v-model.number="spell.ppe"
                      type="number"
                      min="0"
                      :readonly="
                        Boolean(selectedSpell(spell)?.ppeModes?.length)
                      "
                  /></label>
                  <span
                    v-if="spell.ppeText && spell.ppeText !== String(spell.ppe)"
                    class="ppe-note"
                    >Book: {{ spell.ppeText }}</span
                  >
                </div>
                <button
                  class="icon danger"
                  :disabled="chain.spells.length === 1"
                  @click="removeSpell(chain, spell.id)"
                >
                  ×
                </button>
              </div>
            ...
            <div class="chain-result">
              <span
                >Construction P.P.E.
                <strong>{{
                  credits(chainResults[chainIndex].ppeConstruction)
                }}</strong></span
              >
              <span
                >Activation
                <strong>{{
                  chain.mode === 'ley-only'
                    ? 'Ley line required'
                    : `${credits(chainResults[chainIndex].activation)} P.P.E.`
                }}</strong></span
              >
            </div>
          </article>
        ...
        <section class="panel grid modifiers">
          <label
            >P.P.E. storage<input
              v-model.number="state.storagePpe"
              type="number"
              min="0"
          /></label>
          <label
            >Construction increase per stored P.P.E. (%)<input
              v-model.number="state.storagePercentPerPoint"
              type="number"
              min="0"
              step="0.1"
          /></label>
          <label
            >Assistant time reduction (%)<input
              v-model.number="state.assistantTimeReduction"
              type="number"
              min="0"
              max="35"
          /></label>
          <div class="construction-adjustments wide">
            <fieldset class="modifier-options">
              ...
              <p>
                Selected penalties are added together and subtracted from the
                construction skill roll.
              </p>
              <label
                v-for="modifier in RIFTS_CONSTRUCTION_MODIFIERS"
                :key="modifier.id"
                v-memo="[selectedConstructionModifier(modifier.id)?.quantity]"
              ...
              >
                <input
                  ...
                  @change="
                    toggleConstructionModifier(modifier, $event.target.checked)
                  "
                />
                <span
                  ><strong>{{ modifier.label }}</strong>
                  <b class="penalty-percent">-{{ modifier.percent }}%</b
                  ><small v-if="modifier.note">{{ modifier.note }}</small></span
                >
                <span
                  v-if="
                    modifier.repeatable &&
                    selectedConstructionModifier(modifier.id)
                  "
                  class="modifier-quantity"
                >
                  <input
                    ...
                    step="1"
                    @input="
                      setConstructionModifierQuantity(
                        modifier.id,
                        $event.target.value,
                      )
                    "
            ...
            <fieldset class="modifier-options bonus-options">
              ...
              <p>
                Selected bonuses are added together and applied to the
                construction skill roll.
              </p>
              <label
                v-for="bonus in RIFTS_CONSTRUCTION_BONUSES"
                :key="bonus.id"
                v-memo="[selectedConstructionBonus(bonus.id)?.quantity]"
                class="modifier-option"
              >
                <input
                  type="checkbox"
                  ...
                  @change="
                    toggleConstructionBonus(bonus, $event.target.checked)
                  "
                />
                <span
                  ><strong>{{ bonus.label }}</strong>
                  <b class="bonus-percent">+{{ bonus.percent }}%</b
                  ><small v-if="bonus.note">{{ bonus.note }}</small></span
                >
                <span
                  v-if="bonus.repeatable && selectedConstructionBonus(bonus.id)"
                  class="modifier-quantity"
                >
                  <input
          ...
          <label
            >Other custom skill-roll modifier (%)<input
              v-model.number="state.constructionModifierPercent"
              type="number"
              step="1"
          /></label>
          <label
            >Selected bonuses (%)<input
              :value="credits(selectedConstructionBonusPercent)"
              readonly
          /></label>
          <label
            >Total skill-roll modifier (%)<input
              :value="signedCredits(totalSkillRollModifier)"
              readonly
          /></label>
          <label
            >Construction-time adjustment<input
              :value="
                timeAdjustmentValue(
        ...
        <section
          ref="priceChartSection"
          class="panel price-chart"
        >
          <h2>Total price and activation cost by primary gem size</h2>
          <p class="chart-context">
            Varies Function 1's primary gem; other functions remain at their
            entered carat values.
          </p>
          <svg
            v-if="chartVisible"
          ...
          >
            <title id="price-chart-title">
              Total item price and activation cost by primary gem carats
            </title>
            <desc id="price-chart-description">
              Total construction price on the left axis and activation P.P.E. on
              the right axis, from zero to six primary gem carats in half-carat
              increments.
            </desc>
            ...
            >
              ...
              />
              <text
                class="chart-label chart-label-price"
            ...
            <g
              v-for="tick in chart.activationTicks"
              :key="`activation-y-${tick.value}`"
            >
              <text
                class="chart-label chart-label-activation"
                :x="chart.margin.left + chart.plotWidth + 12"
                :y="tick.y + 4"
              >
                {{ credits(tick.value) }}
              </text>
            ...
            />
            <line
              class="chart-axis chart-axis-activation"
            ...
            <polyline
              ...
              :points="
                chart.points
                  .map((point) => `${point.x},${point.priceY}`)
                  .join(' ')
              "
            />
            <polyline
              class="chart-line chart-line-activation"
              :points="
                chart.points
                  .map((point) => `${point.x},${point.activationY}`)
                  .join(' ')
              "
            />
            <polyline
            ...
            >
              <title>
                {{ point.carat }} carats: {{ credits(point.price) }} credits
                total price
              </title>
            </circle>
            <circle
            ...
            >
              <title>
                {{ point.carat }} carats: {{ credits(point.activation) }} P.P.E.
                activation cost
              </title>
            </circle>
            ...
            <text
              class="chart-axis-title chart-axis-title-price"
              :transform="verticalAxisTransform(18, chart, -90)"
              text-anchor="middle"
            >
              Total price (credits)
            </text>
            <text
              class="chart-axis-title chart-axis-title-activation"
              :transform="verticalAxisTransform(chart.width - 18, chart, 90)"
              text-anchor="middle"
            >
              Activation cost (P.P.E.)
            </text>
            ...
            >
              ...
              <circle
                :class="
                  chartTooltip.series === 'price'
                    ? 'chart-point'
                    : 'chart-point-activation'
                "
                cx="14"
                cy="17"
                r="4"
              ...
              <text
                ...
                y="21"
              >
                {{ chartTooltip.point.carat }} carats
              </text>
              <text
              ...
              >
                {{
                  credits(
                    chartTooltip.series === 'price'
                      ? chartTooltip.point.price
                      : chartTooltip.point.activation,
                  )
                }}
          ...
          >
            Chart loads when it approaches the viewport.
          </p>
        </section>
      </div>
      ...
      <aside
        ...
        aria-label="Final device statistics"
      >
        <div class="results-sidebar-inner">
          <h2>Final statistics</h2>
          <div class="stat">
            <span>Total construction P.P.E.</span
            ><strong>{{ credits(summary.ppeConstruction) }}</strong>
          </div>
          <div class="stat">
            <span>Total activation P.P.E.</span>
            <strong>{{
              summary.allFunctionsRequireLeyLine
                ? 'Ley line required'
                : credits(summary.activationPpe)
            }}</strong>
            <small
              v-if="
                summary.hasLeyLineOnlyFunctions &&
                !summary.allFunctionsRequireLeyLine
              "
              >Ley-line functions require a ley line.</small
            >
          </div>
          <div class="stat">
            <span>Construction time</span
            ><strong>{{ credits(summary.constructionHours) }} hours</strong>
          </div>
          <div class="stat">
            <span>Required gems</span
            ><strong>{{ credits(summary.gemsCredits) }} credits</strong>
          </div>
          <div class="stat">
            <span>Cost before gems</span
            ><strong
              >{{
                credits(summary.constructionCreditsBeforeGems)
              }}
              credits</strong
            >
          </div>
          <div class="stat">
            <span>Total construction cost (includes gems)</span
            ><strong>{{ credits(summary.constructionCredits) }} credits</strong>
          </div>
          <div class="stat skill-stat">
            <span>Total skill-roll modifier</span>
            <strong
              :class="
                summary.skillRollModifier < 0
                  ? 'negative'
                  : summary.skillRollModifier > 0
                    ? 'positive'
                    : ''
            ...
            >
              {{ summary.skillRollModifier >= 0 ? '+' : ''
              }}{{ credits(summary.skillRollModifier) }}%
            </strong>
          </div>
        </div>
      </aside>
    ...
    <footer>
      <p>
        <strong>Rules note:</strong> This is a table aid, not a replacement for
        the rulebook. The G.M. still assigns spell chains, effects, gems,
        limitations, and final rulings.
      </p>
    </footer>
  </main>
</template>
