...
.device-image-frame:hover {
  border-color: var(--color-accent);
}
.device-image-frame:focus-visible {
...
.device-image-close {
  ...
  border-radius: 999px;
  background: rgba(10, 17, 37, 0.92);
  color: var(--color-text);
  font-size: 2rem;
  line-height: 1;
  cursor: pointer;
}
.device-image-close:hover {
  border-color: var(--color-accent);
  background: var(--color-surface-raised);
}
.device-image-close:focus-visible {
  outline: 3px solid var(--color-accent);
  outline-offset: 3px;
}
</style>
