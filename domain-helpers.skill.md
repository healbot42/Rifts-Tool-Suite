...
Purpose
- Help AI agents maintain the pure calculator logic already extracted into `src/pages/tw-calculator/lib/calculations.js` while keeping UI orchestration in `components/TwCalculatorApp.vue`.
...
- Prefer adding or updating pure functions in `src/pages/tw-calculator/lib/calculations.js`.
...
- Input and output shapes that match the reactive state in `components/TwCalculatorApp.vue`.
...
- Round only final summary values unless the source rule requires earlier rounding.
